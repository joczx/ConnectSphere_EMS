-- Apply in the Supabase SQL Editor after the events table exists.
-- All authenticated users may reserve equipment, regardless of role/membership.
begin;
...
revoke all on public.equipment, public.equipment_reservations from anon, authenticated;
...
create function public.equipment_peak(p_equipment_id uuid, p_start timestamptz, p_end timestamptz)
...
begin
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
        'equipment_id', q.equipment_id, 'name', q.name, 'total_quantity', q.total_quantity,
    ...
    ) order by q.name), '[]'::jsonb) into items
...
create function public.reserve_equipment(p_event_id text, p_equipment_id uuid, p_quantity integer)
...
revoke all on function public.equipment_events(), public.equipment_peak(uuid,timestamptz,timestamptz),
public.equipment_availability(text), public.reserve_equipment(text,uuid,integer), public.guard_equipment_event() from public, anon, authenticated;
grant execute on function public.equipment_events(), public.equipment_availability(text), public.reserve_equipment(text,uuid,integer) to authenticated;
commit;
