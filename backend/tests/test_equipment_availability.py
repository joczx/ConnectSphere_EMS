...
class EquipmentAvailabilityTests(unittest.TestCase):
    ...
    def test_check_equipment_availability_allows_any_authenticated_user(self, query):
        query.side_effect = [
            {'id': 'tech-user'},
            [
                {'equipment_id': 10, 'equipment_type': 'Projector', 'equipment_model': 'PX-500', 'total_quantity': 5},
            ],
            [],
        ]
    ...
    @patch('app.services.equipment_availability_service.supabase_request')
    def test_check_equipment_availability_reports_insufficient_quantity_and_overlap(self, query):
        query.side_effect = [
            {'id': 'tech-user'},
            [{'role_id': 3}],
            [{'role_name': 'technical_support_staff'}],
            [
                {'equipment_id': 10, 'equipment_type': 'Projector', 'equipment_model': 'PX-500', 'total_quantity': 5},
            ],
            [
                {
                    ...
                    'events': {
                        ...
                        'end_datetime': '2026-10-01T11:30:00Z',
                    },
                }
            ],
        ]
        ...
        self.assertEqual(response.json['results'][0]['status'], 'insufficient')
        self.assertIn('insufficient', response.json['message'].lower())
    ...
    def test_check_equipment_availability_returns_no_suitable_equipment_message(self, query):
        query.side_effect = [
            ...
            [{'role_name': 'technical_support_staff'}],
            [
                {'equipment_id': 11, 'equipment_type': 'Microphone', 'equipment_model': 'M-100', 'total_quantity': 2},
            ],
            [],
        ]
